...
- **比较两个版本**：按住 Cmd（macOS）/ Ctrl 点第二个提交，下面直接给出这两个版本
  之间的全部差异 —— 不管中间隔了多少个提交、跨不跨分支都行。⇄ Swap 反转方向，
  Esc 或 Exit compare 退出。右键菜单里也有 **Compare with …** 这个入口。
...
不会。文件清单和逐行内容是分开取的：先只列出"改了哪些文件"（实测跨 500 个提交、
1094 个文件的清单只有 148 KB），点开哪个文件才去取哪个文件的内容。
把 25 万行 diff 一次性塞给浏览器才会卡，而这条路走不到那儿。
